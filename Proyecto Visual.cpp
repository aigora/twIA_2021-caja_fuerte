...
void Talk_with_Arduino(Serial* Arduino)
{
	...
	if (Arduino->IsConnected()) 
	{
		...
		bytesReceive = Send_and_Receive(Arduino, "GET_COMP", -1, BufferEntrada, &numero_recibido);
		if (bytesReceive == 0)
			printf("No se ha recibido respuesta al mensaje enviado\n");
		else
			printf("%s %d\n", BufferEntrada, numero_recibido);

		bytesReceive = Send_and_Receive(Arduino, "GET_ALERTA", -1, BufferEntrada, &numero_recibido);
		if (bytesReceive == 0)
			printf("No se ha recibido respuesta al mensaje enviado\n");
		else
			printf("%s\n", BufferEntrada);
